...
const InstructorProfile = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      fetch(`http://localhost:3000/courses?instructorEmail=${user.email}`)
        .then(res => res.json())
    ...
    <div className="max-w-5xl mx-auto p-6 mt-10 bg-white rounded-xl shadow-lg border border-blue-100">
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div className="text-center md:text-left">
          <h2 className="text-3xl font-bold text-black">{instructorData.name}</h2>
          <p className="text-gray-600 mt-1"><span className="font-medium">Email:</span> {instructorData.email}</p>
          <p className="text-gray-600"><span className="font-medium">Role:</span> {instructorData.role}</p>
        </div>
      </div>

      <div className="mt-8">
        <h3 className="text-xl font-semibold text-black mb-2">Bio</h3>
        <p className="text-gray-700 leading-relaxed bg-blue-50 p-4 rounded">{instructorData.bio || 'No bio provided.'}</p>
      </div>
      ...
      <div className="mt-10">
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {courses.map(course => (
              <div key={course._id} className="p-4 bg-blue-50 border border-blue-100 rounded-lg shadow-sm hover:shadow-md transition">
